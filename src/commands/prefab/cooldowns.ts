...
export default class Cooldowns extends KelleeBotCommand {
          ...
          execute: async ({ client, interaction }) => {
            ...
            if (!command)
              embed.setDescription(
                `${interaction.user}, there is no command \`${name}\`.`
              );
            else if (command.canNotSetCooldown)
              embed.setDescription(
        ...
        set: {
          ...
          execute: async ({ client, interaction }) => {
            ...
            if (!command)
              ...
              embed.setDescription(
                `${interaction.user}, the cooldown can't be longer than 24h.`
              );
            else if ((command.cooldown ?? 0) === time)
              embed.setDescription(
                `${interaction.user}, that's already the default cooldown for this command.`
              );
            else {
              if (time === 0) {
                ...
                await client.guildInfo.findByIdAndUpdate(
                  ...
                  {
                    $set: {
                      [`commandCooldowns.${command.name}.${role}`]: time * 1000
                    }
                  },
                  { new: true, upsert: true, setDefaultsOnInsert: true }
                );
