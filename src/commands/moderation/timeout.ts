...
export const timeout = async (client: Client, interaction: CommandInteraction) => {
    ...
    if (member.user.bot)
        return interaction.reply({ content: "Bots cannot be timed out.", ephemeral: true });

    if (isNaN(ms(duration)))
        return interaction.reply({ content: "Please specify a valid duration.", ephemeral: true });

    if (ms(duration) < MIN_DURATION)
        return interaction.reply({ content: `Duration must be at least ${ms(MIN_DURATION)}.`, ephemeral: true });

    if (member.id === user.id)
        return interaction.reply({ content: "Nice try. Why would you time yourself out?", ephemeral: true });
    ...
    if (member.isCommunicationDisabled())
        return interaction.reply({
            content: `That member is already timed out. Their timeout will expire <t:${Math.round(member.communicationDisabledUntilTimestamp / 1000)}:R>.`,
            ephemeral: true
        });
    ...
    try {
        const timedoutMember = await member.timeout(ms(duration), reason).catch(e => client.utils.log("ERROR", `${__filename}`, `An error has occurred: ${e}`));
        if (!timedoutMember)
            return msg.edit({ content: `An error has occurred and **${member.user.tag}** was not timed out.` });
        ...
        return msg.edit({
            content: `**${timedoutMember.user.tag}** has successfully been timed out for ${ms(ms(duration), { long: true })}. Their timeout will expire ${expirationTimestamp}.`
        });
