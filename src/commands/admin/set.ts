    ...
    super(client, {
      ...
      options: [
        ...
        },
        {
          name: "user",
          description:
            "The user to set points to. Defaults 'all' if no user specified.",
          type: "USER"
        }
      ],
      execute: async ({ client, interaction }) => {
        ...
        try {
          ...
          if ((member as GuildMember).user.bot)
            return await interaction.reply({
              content: "You cannot set bot's points."
            });

          const newPoints = await setPoints(
            interaction.guildId!,
            (member as GuildMember).id,
            points
          );
          return await interaction.reply({
            content: `Points have been successfully set to \`${client.utils.formatNumber(
              newPoints
            )}\` for **${(member as GuildMember).user.tag}**.`
          });
        } catch (e) {
          client.utils.log(
            "ERROR",
