  ...
  constructor(client: Client) {
    super(client, {
      ...
      execute: async ({ client, interaction }) => {
        ...
        try {
          const res = await client.movieDb.searchTv({
            query: tv,
            page: 1,
            include_adult: false
          });
          if (!res || !res.results || !res.results[0])
            return interaction.reply({
              content: `No results were found for "${tv}".`,
              ephemeral: true
            });
          ...
          if (res.results.length > 1) {
            ...
            const tvEmbed = await showTvInfo(
              client,
              interaction.user.id,
              res.results[0]
            );
...
const showAllShows = async (
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    let firstAirDate = results[i].first_air_date
      ? results[i].first_air_date!.substring(0, 4)
      : "";
    tvList += `${i + 1}. ${title} ${
    ...
    selectMenu.addOptions({
      ...
      description:
        results[i].original_language !== "en" && results[i].original_name
  ...
  const msgEmbed = (
    ...
    .setFooter({
      text: "Select the TV show from the dropdown you want to see information for."
    });
...
const showTvInfo = async (
  ...
  const firstAirDateTimestamp = DateTime.fromISO(
    new Date(`${first_air_date}`).toISOString()
  ).toSeconds();
  const firstAirTimestamp = firstAirDateTimestamp
    ? `<t:${firstAirDateTimestamp}:F> (<t:${firstAirDateTimestamp}:R>)`
    : "";
    ...
    .addFields(
      {
        ...
        value: number_of_seasons ? `${number_of_seasons}` : "0",
        inline: true
      },
      {
        name: "**Number of Episodes**",
        value: number_of_episodes
          ? client.utils.formatNumber(number_of_episodes)
          : "0",
        inline: true
      },
  ...
  if (!in_production) {
    const lastAirDateTimestamp = DateTime.fromISO(
      new Date(`${last_air_date}`).toISOString()
    ).toSeconds();
    const lastAirTimestamp = lastAirDateTimestamp
      ? `<t:${lastAirDateTimestamp}:F> (<t:${lastAirDateTimestamp}:R>)`
      : "";
    msgEmbed.addFields({
      name: "**Last Air Date**",
      value: lastAirTimestamp,
      inline: false
    });
  }
...
const getOriginCountryNameAndLanguage = async (code: string[]) => {
  ...
  for (let i = 0; i < code.length; i++) {
    const resp = await axios.get(
      `https://restcountries.com/v3.1/alpha/${code[i]}`
    );
    countryInfo.push({
      name: resp.data[0].name.common,
      language: Object.values(resp.data[0].languages).join(", ")
    });
  }
  return countryInfo;
};
