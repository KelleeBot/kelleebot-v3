  ...
  constructor(client: Client) {
    super(client, {
      ...
      execute: async ({ client, interaction }) => {
        ...
        try {
          const res = await client.movieDb.searchMovie({
            query: movie,
            page: 1,
            include_adult: false
          });
          if (!res || !res.results || !res.results[0])
            return interaction.reply({
              content: `No results were found for "${movie}".`,
              ephemeral: true
            });
          ...
          if (res.results.length > 1) {
            ...
            const movieEmbed = await showMovieInfo(
              client,
              interaction.user.id,
              res.results[0]
            );
...
const showAllMovies = async (
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    let releaseYear = results[i].release_date
      ? results[i].release_date?.substring(0, 4)
      : "";
    movieList += `${i + 1}. ${title} ${
    ...
    selectMenu.addOptions({
      ...
      description:
        results[i].original_language !== "en" ? results[i].title : "",
      value: i.toString()
    });
  }
  ...
  const msgEmbed = (
    ...
    .setFooter({
      text: "Select the movie from the dropdown you want to see information for."
    });
...
const showMovieInfo = async (
  ...
  const { budget, genres, revenue, runtime, homepage, production_companies } =
    movieInfo;
  ...
  return (await client.utils.CustomEmbed({ userID }))
    ...
    .addFields(
      ...
      {
        name: "**Budget**",
        value: `$${client.utils.formatNumber(budget!)}`,
        inline: true
      },
      {
        name: "**Revenue**",
        value: `$${client.utils.formatNumber(revenue!)}`,
        inline: true
      },
