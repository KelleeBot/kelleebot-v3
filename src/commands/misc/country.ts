...
const fetchCountry = async (name: string) => {
    const resp = await axios.get(`https://restcountries.com/v3.1/name/${encodeURIComponent(name)}`);
    return resp.data;
};

const showCountryInfo = async (client: Client, userID: Snowflake, country: any) => {
    const { name, population, region, subregion, continents, capital, demonyms, area, cca3, languages, flags } = country;

    const countryNativeName = name.nativeName ? Object.values(name.nativeName).map((n: any) => `${n.official}`)[0] : name.official;
    const countryDemonyms = Object.entries(demonyms.eng)
        .map((d) => `${d[0].toUpperCase()}: ${d[1]}`)
        .join("\n");
    const countryAreaM = area !== "-" ? client.utils.formatNumber(parseFloat((area * 0.62137).toFixed(2))) : "-";
    const countryCurrencyName = country.currencies
        ? Object.values(country.currencies)
              .map((c: any) => `${c.name}`)
              .join(", ")
        : "-";
    const countryCurrencyCode = country.currencies ? Object.keys(country.currencies).join(", ") : "-";
    ...
    const msgEmbed = (await client.utils.CustomEmbed({ userID }))
        ...
        .addFields(
            ...
            {
                name: "**Area**",
                value: area === "-" ? "-" : `${client.utils.formatNumber(area)}km (${countryAreaM}m)`,
                inline: true
            }
        )
