...
export const setlog = async (client: Client, interaction: CommandInteraction) => {
    ...
    if (channel.type !== "GUILD_TEXT")
        return interaction.reply({ content: "Only text channels can be set as the log channel.", ephemeral: true });

    if (channel.isThread())
        return interaction.reply({ content: "Thread channels are not allowed to be set as the log channel.", ephemeral: true });
