...
export const dog = async (client: Client, interaction: CommandInteraction) => {
    ...
    try {
        const resp = await axios.get("https://some-random-api.ml/animal/dog");
        const { data } = resp;
        ...
        const msgEmbed = (await client.utils.CustomEmbed({ userID: interaction.user.id }))
            ...
            .setURL(data.image)
            .setImage(data.image)
            .addField("**Random Dog Fact**", data.fact, true);
        ...
        return interaction.editReply({
            content: "Oops! Looks like an error occurred. No doggo pics as of right now :("
        });
    }
};
