...
export const softban = async (client: Client, interaction: CommandInteraction) => {
    ...
    try {
        ...
        if (!msg) return interaction.reply({ content: `An error has occurred and **${member.user.tag}** was not soft banned. Please try again.`, ephemeral: true });
        ...
        collector.on("collect", async (button) => {
            if (button.customId === "softban_yes") {
                ...
                await button.editReply({ content: `**${member.user.tag}** was not soft banned.`, embeds: [], components: [] });
            }
            collector.stop();
        });

        collector.on("end", async (_collected, reason) => {
            if (reason === "time") await msg.edit({ content: `You did not choose a response in time. **${member.user.tag}** was not soft banned.`, embeds: [], components: [] });
        });
    } catch (e: any) {
        client.utils.log("ERROR", `${__filename}`, `An error has occurred: ${e.message}`);
        return interaction.reply({ content: `An error has occurred and **${member.user.tag}** was not soft banned. Please try again.`, ephemeral: true });
    }
};

const softbanUser = async (member: GuildMember, button: ButtonInteraction, client: Client, reason: string) => {
    try {
        const msg = await button.editReply({ content: `Soft banning **${member.user.tag}**...`, embeds: [], components: [] }) as Message;

        const ban = await button.guild!.members.ban(member, { days: 7, reason });
        if (!ban) return msg.edit({ content: `An error occurred. Please try again. **${member.user.tag}** was not soft banned.`, embeds: [], components: [] });

        const unban = await button.guild!.members.unban(member, reason);
        if (!unban) return msg.edit({ content: `An error occurred. Please try again. **${member.user.tag}** was not soft banned.`, embeds: [], components: [] });
        ...
        await msg.edit({ content: `Successfully soft banned **${member.user.tag}** from ${button.guild!.name}.`, embeds: [], components: [] });
