...
export const ban = async (client: Client, interaction: CommandInteraction) => {
    ...
    if (user.id === interaction.user.id) return interaction.reply({ content: "Now why would you want to ban yourself?", ephemeral: true });

    if (user.id === client.user?.id) return interaction.reply({ content: "Nah, I don't think so. I'm un-bannable.", ephemeral: true });
