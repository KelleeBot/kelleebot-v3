...
export const unban = async (
  ...
  if (!guildBans.size)
    return interaction.reply({
      content: "This server doesn't have anyone banned.",
      ephemeral: true
    });
  ...
  if (!bannedMember)
    return interaction.reply({
      content: "This user is currently not banned from this server.",
      ephemeral: true
    });
    ...
    collector.on("collect", async (button) => {
      if (button.customId === "unban_yes") {
        ...
        await button.editReply({
          content: `**${bannedMember.user.tag}** was not unbanned.`,
          embeds: [],
          components: []
        });
      }
...
const unbanUser = async (
  ...
  const unbanMsg = (await button.editReply({
    content: `Unbanning **${bannedUser.tag}**...`
  })) as Message;
  const user = await button.guild!.members.unban(bannedUser, reason);

  if (!user)
    return await unbanMsg.edit({
      content: `There was an error and **${bannedUser.tag}** was not unbanned. Please try again.`
    });
