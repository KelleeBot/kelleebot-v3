...
export default async (
  ...
  if (oldMember.nickname !== newMember.nickname) {
    ...
    const msgEmbed = createEmbed(
      client,
      GUILD_MEMBER_EVENTS as ColorResolvable,
      `**${newMember.user} nickname changed**`,
      {
    ...
    const fetchedLog = await client.utils.fetchAuditLog(
      newMember.guild!,
      "MEMBER_UPDATE"
    );
    if (fetchedLog) {
...
const roleUpdatedLog = async (
  ...
  const fetchedLogs = await client.utils.fetchAuditLog(
    role.guild,
    "MEMBER_ROLE_UPDATE"
  );
  if (!fetchedLogs) {
    const msgEmbed = createEmbed(
      client,
      GUILD_MEMBER_ROLES as ColorResolvable,
      description,
      { user }
    );
...
const createEmbed = (
  ...
  return client.utils
    ...
    .setAuthor({
      ...
      iconURL: options.user
        ? options.user.displayAvatarURL({ dynamic: true })
        : client.utils.getGuildIcon(options.guild!)
    })
    .setDescription(description)
    .setFooter({
      text: `ID: ${options.user ? options.user.id : options.guild?.id}`
    })
    .setTimestamp();
};
