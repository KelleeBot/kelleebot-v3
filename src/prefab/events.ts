...
const getDefaultChannel = (guild: Guild): TextChannel => {
    ...
    channel = guild.channels.cache.find(
        (channel) => channel.name === "general" && channel.permissionsFor(guild.me!).has("SEND_MESSAGES") && channel.type === "GUILD_TEXT"
    );
    //@ts-ignore
    ...
    return (
        guild.channels.cache
            .filter((c) => c.type === "GUILD_TEXT" && !c.isThread && c.permissionsFor(guild.me!).has("SEND_MESSAGES"))
            //@ts-ignore
            ...
            if (!client.config.DEVS.includes(interaction.user.id)) {
                if (command.guildOnly) {
                    ...
                    if (command.ownerOnly && interaction.guild!.ownerId !== interaction.user.id)
                        return await quickReply(client, interaction, "This command can only be used by the server owner.");

                    const guildInfo = await client.guildInfo.get(interaction.guildId);

                    if (guildInfo.disabledCommands.includes(command.name))
                        return await quickReply(client, interaction, "This command is currently disabled in this server.");
                    if (guildInfo.disabledChannels.includes(interaction.channelId) && !command.ignoreDisabledChannels)
                        return await quickReply(client, interaction, "You can't use any commands in this channel.");
                    ...
                    if (command.clientPerms && !interaction.channel!.permissionsFor(interaction.guild!.me!).has(command.clientPerms, true))
                        return await quickReply(
                            client,
                            interaction,
                            `I am missing the following permissions: ${client.utils.missingPermissions(interaction.guild!.me!, command.clientPerms)}.`
                        );
                    ...
                    if (
                        ...
                        return await quickReply(
                            client,
                            interaction,
                            `You are missing the following permissions: ${client.utils.missingPermissions(member, command.perms)}.`
                        );

                    if (command.nsfw && (interaction.channel!.isThread() || !interaction.channel!.nsfw))
                        return await quickReply(client, interaction, `This command may only be used in an NSFW channel.`);
                }
