    ...
    if (interaction.isCommand()) {
      ...
      if (!client.config.DEVS.includes(interaction.user.id)) {
        if (command.guildOnly) {
          ...
          if (guildInfo.disabledCommands.includes(command.name)) return await quickReply(client, interaction, "This command is currently disabled in this server.");
          if (guildInfo.disabledChannels.includes(interaction.channelId) && !command.ignoreDisabledChannels) return await quickReply(client, interaction, "You can't use any commands in this channel.");
          ...
          if (guildInfo.commandPerms && guildInfo.commandPerms[command.name] && !member.permissions.has(guildInfo.commandPerms[command.name], true)) return await quickReply(client, interaction, `You are missing the following permissions: ${client.utils.missingPermissions(member, guildInfo.commandPerms[command.name])}.`);
