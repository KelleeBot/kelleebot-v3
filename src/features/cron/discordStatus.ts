...
import { StatusPageIncident, StatusPageResult } from "../../types/discordStatus";
import { EMBED_COLOR_BLACK, EMBED_COLOR_GREEN, EMBED_COLOR_ORANGE, EMBED_COLOR_RED, EMBED_COLOR_YELLOW } from "../../../config/embedColours.json";
import discordIncidents from "../../schemas/discordIncidents";

const webhook = new WebhookClient({ url: `${process.env.TWITCH_WEBHOOK_URL}` });

export default (client: Client) => {
    new cron.CronJob("00 */5 * * * *", () => execute(client), null, true, "America/Denver");
};

const execute = async (client: Client) => {
    try {
        const json = await axios.get("https://srhpyqt94yxb.statuspage.io/api/v2/incidents.json");

        const { incidents } = json.data as StatusPageResult;
        for (const incident of incidents.reverse()) {
            const data = await discordIncidents.findById(incident.id);
            if (!data) {
        ...
        client.utils.log("ERROR", `${__filename}`, `Error during fetching and update routine: ${e}`);
    }
};
const updateIncident = async (client: Client, incident: StatusPageIncident, messageID?: Snowflake) => {
    ...
    try {
        ...
        await discordIncidents.findOneAndUpdate(
            {
                _id: incident.id
            },
            {
        ...
        if (messageID) {
            return client.utils.log("ERROR", `${__filename}`, `Error during webhook update on incident: ${incident.id}, message: ${messageID}\n${e}`);
        }
        client.utils.log("ERROR", `${__filename}`, `Error during webhook sending on incident: ${incident.id}\n${e}`);
    }
};
...
const embedFromIncident = (client: Client, incident: StatusPageIncident) => {
    ...
    const embed = client.utils
        ...
        .setAuthor({ name: "Discord", iconURL: "https://cdn.discordapp.com/emojis/936023766495031296.png" })
